...
const Ventas = () => {
  ...
  const obtenerNombreCliente = async (idCliente) => {
    if (!idCliente) return '—';
    try {
      const resp = await fetch(`http://localhost:3001/api/cliente/${idCliente}`);
      if (!resp.ok) return '—';
  ...
  const obtenerNombreEmpleado = async (idEmpleado) => {
    ...
    try {
      const resp = await fetch(`http://localhost:3001/api/empleados/${idEmpleado}`);
      if (!resp.ok) return '—';
      const data = await resp.json();
      return `${data.primer_nombre} ${data.primer_apellido}`;
    } catch (error) {
  ...
  const obtenerNombreProducto = async (idProducto) => {
    ...
    try {
      const resp = await fetch(`http://localhost:3001/api/productos/${idProducto}`);
      if (!resp.ok) return '—';
      const data = await resp.json();
  ...
  const obtenerVentas = async () => {
    try {
      const resp = await fetch('http://localhost:3001/api/ventas');
  ...
  const obtenerEmpleados = async () => {
    try {
      const resp = await fetch('http://localhost:3001/api/empleados');
  ...
  const actualizarVenta = async () => {
    const total = detallesNuevos.reduce((sum, d) => sum + (d.cantidad * d.precio_unitario), 0);
    try {
      await fetch(`http://localhost:3001/api/actualizarventa/${ventaAEditar.id_venta}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      for (const d of actuales) {
        await fetch(`http://localhost:3001/api/eliminarDetalleVenta/${d.id_detalle_venta}`, { method: 'DELETE' });
      }
      for (const d of detallesNuevos) {
        await fetch('http://localhost:3001/api/registrarDetalleVenta', {
          method: 'POST',
  ...
  const eliminarVenta = async () => {
    try {
      await fetch(`http://localhost:3001/api/eliminarventa/${ventaAEliminar.id_venta}`, { method: 'DELETE' });
      await obtenerVentas();
